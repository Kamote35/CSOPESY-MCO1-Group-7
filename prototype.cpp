...
void handleReportUtil() {
    lock_guard<mutex> lock(processLock);
    ofstream out("csopesy-log.txt", ios::out | ios::trunc);

    if (!out.is_open()) {
        cout << "Error: Unable to write to csopesy-log.txt\n";
        return;
    }
    ...
    int usedCores = min(running, g_numCPU);
